...
public class CameraManager : ICameraManager {
	...
	private unsafe void SetupCameras() {
		...
		this.Active = this.Default = new EditorCamera(this) {
			Name = (Ktisis.Locale.Translate("camera.main")),
	...
	private void SetupWorkCamera() {
		this.WorkCamera ??= new WorkCamera(this, this._context) { Name = Ktisis.Locale.Translate("camera.work") };
	...
	private string GetNextAvailableName() {
		for (var i = this.CameraList.Count + 1; i <= 100; i++) {
			var name = $"{Ktisis.Locale.Translate("camera.camera")} #{i}";
			if (this.CameraList.Any(camera => camera.Name == name))
				continue;
			return name;
		}
		return Ktisis.Locale.Translate("camera.new");
	}
