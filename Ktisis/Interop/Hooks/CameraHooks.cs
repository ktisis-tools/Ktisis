...
namespace Ktisis.Interop.Hooks {
	internal static class CameraHooks {
		...
		private unsafe delegate Matrix4x4* LoadMatrixDelegate(RenderCamera* camera, Matrix4x4* matrix);
		...
		private unsafe static Matrix4x4* CalcViewMatrixDetour(SceneCamera* camera) {
			...
			try {
				...
				if (active != null && camera == &active->CameraBase.SceneCamera) {
					...
					*tarMatrix = matrix;
					LoadMatrix(camera->RenderCamera, tarMatrix);
				}
			} catch (Exception e) {
				Ktisis.Log.Error(e.ToString());
				DisableHooks();
		...
		internal unsafe static void Init() {
			...
			var camVf18 = ((nint*)Services.SigScanner.GetStaticAddressFromSig("48 8D 05 ?? ?? ?? ?? C7 83 ?? ?? ?? ?? ?? ?? ?? ?? 48 89 03 0F 57 C0 33 C0 48 C7 83 ?? ?? ?? ?? ?? ?? ?? ??"))[18];
            TargetHook = Services.Hooking.HookFromAddress<TargetDelegate>(camVf18, TargetDetour);
