...
public class PoseFile : JsonFile {
	...
	public void ConvertLegacyBones() {
		...
		var keys = this.Bones.Keys.ToList();
		foreach (var name in keys) {
			if (AnamLegacyConversions.TryGetValue(name, out var newName)) {
				if (!isLegacy) isLegacy = true;
				this.Bones.Remove(name, out var value);
				if (value != null) this.Bones[newName] = value;
