...
namespace Ktisis.Data.Files {
    public class PoseFile : JsonFile {
		...
		public void ConvertLegacyBones() {
			...
			var keys = Bones.Keys.ToList();
			foreach (var name in keys) {
				if (LegacyConversions.TryGetValue(name, out var newName)) {
					if (!isLegacy) isLegacy = true;
					Bones.Remove(name, out var val);
					if (val != null) Bones[newName] = val;
				} else if (!isLegacy) break;
			}
		}

		public static Dictionary<string, string> LegacyConversions = new() {
			{ "Root", "n_root" },
			{ "Abdomen", "n_hara" },
			{ "Throw", "n_throw" },
			{ "Waist", "j_kosi" },
